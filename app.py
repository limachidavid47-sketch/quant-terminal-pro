...
# ==========================================
st.set_page_config(page_title="Quant Elite V78.2", layout="wide", initial_sidebar_state="expanded")
...
def check_password():
    ...
    st.markdown("<p style='color:#64748B; margin-bottom:20px; text-align: center;'>MOTOR DE RESILIENCIA | LOL INTACTO | DOTA TRIFÁSICO</p>", unsafe_allow_html=True)
...
# ==========================================
# 3. EL CEREBRO QUANT (DOTA TRIFÁSICO)
# ==========================================
...
def motor_dota_trifasico(team_id, match_id=None):
    ...
    p_early = wr_total_ajustado * 0.92  
    p_mid = wr_total_ajustado * 0.96    
    p_late = wr_total_ajustado * 1.02   
    return max(0.05, min(0.95, p_early)), max(0.05, min(0.95, p_mid)), max(0.05, min(0.95, p_late))

# ==========================================
# 4. EL CEREBRO QUANT (LOL INTACTO)
# ==========================================
...
def get_team_stats(team_name, team_id, df_completo):
    ...
    avg_ckpm = df_team['ckpm'].mean() if 'ckpm' in cols else 0
...
if not partidos_filtrados: 
    ...
    for i, m in enumerate(partidos_filtrados[:20]):
        ...
        # EL RELOJ ESTÁ INTACTO (UTC-4, BOLIVIA)
...
""", unsafe_allow_html=True)
            ...
            # INTERFAZ PARA OTROS JUEGOS (DOTA / FPS - VISUALMENTE INTACTOS V78)
...
<div class="glass-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        ...
        <div style="display: flex; gap: 15px; align-items: center;">{boton_stream} {badge}</div>
    </div>
    <div style="display: flex; justify-content: space-around; align-items: center; text-align: center;">
        <div style="width: 35%;">
            ...
            <div style="margin-top:5px;">{"".join([f"<div class='tower-plate {x}'></div>" for x in form1])}</div>
        </div>
        <div style="font-size: 26px; font-weight: bold; color: {c_acc};">VS</div>
        <div style="width: 35%;">
            ...
            <div style="margin-top:5px;">{"".join([f"<div class='tower-plate {x}'></div>" for x in form2])}</div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
            ...
            with st.container(border=True):
                ...
                # --- INYECCIÓN MATEMÁTICA DOTA 2 TRIFÁSICA ---
                if juego_sel == "Dota 2":
                    mercados_g = ["-- Seleccione --", "⭐ Ganador (Late)", "⚖️ Handicap (+1.5) (Late)", "🏁 Carrera a 10 Kills (Early)", "👾 Primer Roshan (Mid)"]
                elif juego_sel == "Mobile Legends":
                    mercados_g = ["-- Seleccione --", "Ganador", "Handicap", "Primera Sangre", "Carrera a 10 Kills", "Total Kills", "Duración"]
                else: 
                    mercados_g = ["-- Seleccione --", "Ganador", "Handicap", "Total Kills", "Primera Sangre", "Carrera a 10 Kills"]
                ...
                if sel_m != "-- Seleccione --":
                    ...
                    # DESVÍO A MOTOR DOTA 2 TRIFÁSICO + MAESTRÍA
                    if juego_sel == "Dota 2":
                        e1, m1, l1 = motor_dota_trifasico(t1['id'], m.get('id'))
                        e2, m2, l2 = motor_dota_trifasico(t2['id'], m.get('id'))
                        
                        if "Early" in sel_m or "10 Kills" in sel_m: p_raw = e1 / (e1 + e2) if t1['name'] in op_sel else e2 / (e1 + e2)
                        elif "Mid" in sel_m or "Roshan" in sel_m: p_raw = m1 / (m1 + m2) if t1['name'] in op_sel else m2 / (m1 + m2)
                        else: p_raw = l1 / (l1 + l2) if t1['name'] in op_sel else l2 / (l1 + l2)
                        
                        if "Handicap" in sel_m: p_raw = min(0.95, p_raw + 0.18) 
                        prob_base = p_raw
                    else:
                        cat = "⚔️ MOBAs" if juego_sel in ["Mobile Legends"] else "🔫 FPS"
