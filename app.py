...
st.markdown("""
    ...
    /* Tarjetas Oscuras */
    .glass-card {
        background: #16181E; padding: 25px; border-radius: 12px; text-align: center;
    ...
    input[type="number"], input[type="password"], input[type="text"] { 
        background-color: #0E1117; border: 1px solid #2D323E; color: white; text-align: center; font-weight: bold;
    }
    
    /* Textos */
    .text-muted { color: #9AA0A6; font-size: 14px; }
    .text-highlight { color: #8AB4F8; font-weight: bold; }
...
# --- MOTOR DE API PANDASCORE ---
st.sidebar.markdown("---")
st.sidebar.markdown("### 🔌 Conexión a Servidor")

api_key = st.sidebar.text_input("🔑 API Key (PandaScore):", type="password")
    ...
    if api_key:
        ...
        try:
            ...
            if respuesta.status_code == 200:
                ...
                st.sidebar.success(f"✅ ¡Conectado! {len(datos)} partidos encontrados.")
                # Muestra los datos crudos del primer partido para confirmar
                st.sidebar.json(datos[0]) 
            else:
                st.sidebar.error(f"❌ Error del servidor: {respuesta.status_code}")
        except Exception as e:
            st.sidebar.error("❌ Fallo de red. Verifica tu conexión.")
    else:
        st.sidebar.warning("⚠️ Ingresa tu API Key primero.")
        
st.sidebar.markdown("---")

nav_cols = st.columns(3)
with nav_cols[0]:
    if st.button("🔴 RADAR EN VIVO"): st.session_state.pagina = "Radar"
with nav_cols[1]:
    if st.button("🏆 EXPLORADOR"): st.session_state.pagina = "Explorador"
with nav_cols[2]:
    if st.button("📊 DIARIO"): st.session_state.pagina = "Diario"
st.markdown("---")
...
if st.session_state.pagina == "Radar":
    ...
    # Partidos simulados (pronto los cambiaremos por los de la API)
    partidos_vivo = [
    ...
    for i, p in enumerate(partidos_vivo):
        with r_cols[i % 2]:
            ...
            if "Más/Menos" in mercado:
                ...
                with c_linea: linea = st.number_input("Handicap:", value=-5.5, step=0.5, key=f"hl_{i}")
                ...
                equipo_sel = st.selectbox("Equipo:", [p['t1'], p['t2']], key=f"es_{i}")
            ...
            if cuota > (1/prob):
                ...
                st.markdown(f"<div style='background:#10B981; color:#0E1117; padding:8px; border-radius:6px; margin-bottom:10px;'><b>Stake: {stake:.2f} U</b></div>", unsafe_allow_html=True)
                if st.button("🚀 Confirmar Apuesta", key=f"b_{i}", use_container_width=True):
                    gestionar_bankroll(bank_actual - stake)
                    st.rerun()
            elif cuota > 1.00:
                st.markdown(f"<p class='text-danger'>❌ Cuota Mínima: {(1/prob):.2f}</p>", unsafe_allow_html=True)
    ...
    st.write("Base de datos de equipos conectada. (En construcción para la API).")
